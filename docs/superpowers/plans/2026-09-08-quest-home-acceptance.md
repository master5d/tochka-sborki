...
| глава | RU | EN |
|---|---|---|
| hero | замер не получен (500 эмбеддера) | edge 0.3274 |
...
13 кадров из 14; расклад тот же, что в первом проходе: иллюстративные главы ближе к образцу,
текстовые дальше, ни один не `inside`. Порог по-прежнему НЕ калиброван — это вторая точка, а не
вердикт о макете.

Прошлый открытый пункт «boulder без замера» ЗАКРЫТ: кадр промерен (outside 0.3548). На его месте
новый и другой: `ru-desktop-hero.png` роняет эмбеддер зверя воспроизводимо (4 из 4), при этом
тот же кадр в 448×280 и обе его половины проходят, а другой кадр той же вьюпорты в 512×320
проходит. Это отказ прибора, не свойство страницы; разбор — в NAUTILUS BACKLOG (зверь доступен
только по LAN, из командировки лог не снять). Побочно там же закрыт наследуемый фолбэк: пул
эмбеддингов спрашивал вектор у чат-моделей и отдавал клиенту три отказа вместо одного.

          ...
          <div key={z.slug} id={`quest-${z.slug}`} style={{
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <Link href={z.href} style={{ color: z.status === 'todo' ? 'var(--text-secondary)' : 'var(--text-primary)', fontWeight: 600, fontSize: '.95rem', textDecoration: 'none' }}>
                {z.questTitle}{z.isNiche && <span style={{ marginLeft: '.5rem', fontFamily: 'var(--font-mono)', fontSize: '.55rem', textTransform: 'uppercase', letterSpacing: '.1em', color: 'var(--text-on-accent)', background: accent, borderRadius: 3, padding: '1px 6px' }}>{nicheLabel}</span>}
              </Link>
              <div style={{ fontSize: '.72rem', color: 'var(--text-secondary)', marginTop: '.15rem' }}>{z.moduleTitle}{z.durationLabel ? ` · ${z.durationLabel}` : ''}</div>
              {cur && <Link href={z.href} style={{ display: 'inline-block', marginTop: '.6rem', background: accent, color: 'var(--text-on-accent)', fontWeight: 900, fontFamily: 'var(--font-mono)', fontSize: '.72rem', textTransform: 'uppercase', letterSpacing: '.06em', padding: '.45rem 1rem', borderRadius: 6, textDecoration: 'none' }}>{cont}</Link>}
            </div>
          </div>
        )
      })}

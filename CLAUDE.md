...
- **Скорочтение = ОТДЕЛЬНЫЙ изолированный hybrid-эпик** (курс+интерактивные тренажёры): S1 dark skeleton SHIPPED в `lib/speedreading/` + `app/speedreading/` (тот же isolation-паттерн, что speech — НЕ `content/`). `lib/speedreading/course.ts` engine+keyed-data (6 уроков `baseline→regression→subvocalization→peripheral→comprehension→retention` + `resolveSpeedreadingCourse`) + `/speedreading`+`/en/speedreading` dark syllabus («готовится», noindex, НЕ в nav/registry). Метод public-domain (Schulte/регрессии/субвокализация/периферийка/Эббингауз — reference shaleny-ravlyk только структурно, ноль чужого копирайта). Вся копи `lintDehustle []`, анти-хайп (без «×2-3»). Sovereign (ноль backend/LLM). **S2 RSVP-читалка SHIPPED** (`/speedreading/rsvp`+en, noindex): pure `lib/speedreading/rsvp.ts` (tokenize/orpIndex/splitOrp Spritz-pivot/buildSchedule + пунктуац-дволл) + `rsvp-store.ts`/`use-rsvp.ts` localStorage `speedreading_rsvp` (зеркало pacing) + `components/speedreading/rsvp-reader.tsx` `'use client'` (ORP на центр-оси monospace `1ch`, WPM-слайдер/chunk/play). Next-слайсы: Schulte · WPM-тест вход/выход · CS-прогресс (RSVP-сессии уже логируются). Детали → [[project_mc_hub_speedreading]].
...
- Опрос в конце модуля (post-class eval, Likert recommend/impact/apply + open, **skippable**) → `<ModuleSurvey module="01-introduction" locale="ru">` (`'use client'`, pre-filled модуль, POST `/api/feedback`→D1, localStorage `module-survey:<id>` не нудит; засеян в модуль 01 u4; fb_6a22ff6e0f0a). Shared `LikertScale` (`required?` default true) переиспользуется standalone `FeedbackForm` (/feedback). ⚠ `/api/feedback` handler требует только `lesson` (Likert опциональны — skippable); standalone форма шлёт всё через client-`required`. `/api/feedback` пишет в D1 `course_feedback` (mig 0013, **direct, БЕЗ n8n** — fb_df698341da2a done); `N8N_*` убран из `Env`+секретов воркера → n8n больше нигде в mc_hub.
- Consent-first форма захвата интереса (event/retreat-лид) → `<CaptureFormBlock id="retreat-inner-evolution" locale="ru">` (string-prop wrapper ← `lib/content/capture-forms.ts getCaptureForm`; `'use client'` `CaptureForm`-engine: inline `<section>` **НЕ модалка**, honeypot `company`, phone опц.+обоснование, GDPR-consent обяз.+гейтит submit, city select-или-free-text; fb_667daeba55b3). POST `/api/leads/capture` → **direct D1+Resend, без n8n**: honeypot→200 noop / bad email|!consent→400 / `INSERT event_leads` (mig 0012) + new-row-only upsert `users` (source=`capture:<event>`) + `ctx.waitUntil(addResendContact)`. Тот же engine+`lib/content`+string-prop паттерн.
...
- Новый blog-пост/заметка → `cd blog && npm run new:post -- <kebab-slug> [--kind note]` (скаффолдит component+2 route, печатает Post-stub для `blog/lib/posts.ts`); заметки (`kind:'note'`) graph-only с плотным `related[]`. Engine+data UI (диаграммы/карточки) — generic builder в `lib/` + курс-данные (`lib/course/*` `Bi{ru;en}`+resolver) + тонкий компонент; рендерить «тёмным» при пустых данных (`amaRegisterUrl:''`, `PRODUCTS=[]`, `REAL_CASES.length>0`).

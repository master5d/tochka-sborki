...
- Опрос в конце модуля (post-class eval, Likert recommend/impact/apply + open, **skippable**) → `<ModuleSurvey module="01-introduction" locale="ru">` (`'use client'`, pre-filled модуль, POST `/api/feedback`→n8n, localStorage `module-survey:<id>` не нудит; засеян в модуль 01 u4; fb_6a22ff6e0f0a). Shared `LikertScale` (`required?` default true) переиспользуется standalone `FeedbackForm` (/feedback). ⚠ `/api/feedback` handler теперь требует только `lesson` (Likert опциональны — skippable); standalone форма шлёт всё через client-`required`. ⚠ `/api/feedback` — **последний живой потребитель n8n** (`feedback.ts`→`N8N_WEBHOOK_URL`, 502 если мёртв); миграция на direct-D1 трекается `fb_df698341da2a`.

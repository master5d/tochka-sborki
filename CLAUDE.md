...
## Сайты (один репо; domain split 2026-07-30: учебное → synergify.com, личное → mamaev.coach)
- **`synergify/`** → `synergify.com` — зонтик-worker `synergify-home`: лендинг с двумя входами (Точка Сборки / академия) + подписка `/api/subscribe` → listmonk + BIMI-логотип.
- **`academy/`** → `academy.synergify.com` — академия S.A.S.H.A («закрытая школа», вход после Точки Сборки; admission-гейт server-verified в workers). Мини-апп (зеркало mentor), читает `LMS/registry.json` fs-bridge'ем (CF Pages `synergify-academy`).
- **`LMS/tochka-sborki/web/`** → `ai.synergify.com` — LMS курса (проект `tochka-sborki`). `public/` хостит one-liner установщики (раздаются по `ai.synergify.com/install.sh` и т.д., запуск одной строкой через curl/irm): `install.sh`/`install.ps1` (Claude-стек: Node+Git+Claude Code) и `install-gfw.sh`/`.ps1` (GFW cloud-relay: Python+LiteLLM proxy:4000+Aider); точки входа — в `02-setup-guide`/`u3-behind-gfw`.
...
- **`workers/`** → `ai.synergify.com/api/*` (основной) **+ legacy `ai.mamaev.coach/api/*` + `mamaev.coach/api/*`** (apex-route для hub events-формы; тот же worker `tochka-sborki-api` на 3 роута, relative `/api/*` долетает с hub) — CF Worker (auth magic-link, progress, feedback, CRM, **Telegram Mini App + companion bot**, **Stripe support checkout**, daily-nudge **cron**). Все три сайта bilingual (RU `/`, EN `/en/`).
...
│       web/              — Next.js 16 LMS сайт (ai.synergify.com), см. web/README.md

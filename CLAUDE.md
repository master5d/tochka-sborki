...
- **A11y baseline** (`lib/a11y/`, `app/globals.css`): keyboard-foundations — глобальный `:focus-visible` outline (keyboard-only, не `:focus`) + `prefers-reduced-motion` guard + skip-to-content (`SkipLink` первым в `Nav` через fragment → `#main-content` на 6 core-shells mdx/module/unit/home/certificate/lesson-layout; fb_a2c667a36a64). **Contrast drift-guard** `lib/a11y/contrast.test.ts` (pure `contrastRatio` util) парсит `model-kit.css` оба `[data-theme]` блока → asserts WCAG-AA ≥4.5 на 12 token-парах; правка-токена-ниже-AA ломает CI (light `--text-accent` #0077cc→#0070c0; fb_8cac8d78f49c). Defer follow-ons: captions (fb_9563271d), low-bandwidth (fb_731d3ee7), broad-ARIA. ⚠ **4 копии** `model-kit.css` (blog/hub/LMS/mentor) — фикс токена в одной НЕ пропагирует.
- **Сертификат** (`/certificate`, `components/certificate-svg.tsx` ← engine+data `lib/course/certificate.ts` `resolveCertificate`+`CERT_PALETTE`): «золотой билет» gold-on-dark, **symbolic** milestone (НЕ обещание доступа в S.A.S.H.A-академию); publisher «Mamaev Institute for AI» НЕ переименовывать тут (fb_7f1d36587e18 отд., заблокирован non-profit-регистрацией); fb_6ded7b0b7980.

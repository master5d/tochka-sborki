...
- **Сертификат** (`/certificate`, `components/certificate-svg.tsx` ← engine+data `lib/course/certificate.ts` `resolveCertificate`+`CERT_PALETTE`): «золотой билет» gold-on-dark; **admission настоящий** — страница fire-and-forget POST `/api/academy/admission` (S4), билет рендерится идентично при fail/401 (fb_6ded7b0b7980); publisher «Mamaev Institute for AI» НЕ переименовывать тут (fb_7f1d36587e18 отд., заблокирован non-profit-регистрацией).

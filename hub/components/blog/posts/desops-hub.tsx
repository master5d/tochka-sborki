...
export function DesOpsHub({ locale }: Props) {
  ...
  return (
    <div className={styles.prose}>
      ...
      <p>
        {'Мы наблюдаем фундаментальный сдвиг: дизайн перестает быть отдельной профессией и превращается в <b>загружаемый навык</b> для вашего код-агента. Проекты вроде <b>open-design</b> от Tom Huang (open-source клон Claude Design) позволяют Claude Code или Cursor подгружать 30+ дизайн-скиллов и 70+ топовых брендовых систем в один клик.'}
      </p>

      <h2>5. «Я скачал себе дизайнера»: Дизайн-Протоколы</h2>
      <p>
        {'Ключевой инсайт последних месяцев — использование <b>Дизайн-Протоколов</b> (например, методологии <b>huashu-design</b>). Мы больше не просим ИИ «сделать красиво». Мы загружаем методологию как скилл, и агент начинает действовать системно, как по «еб***ему протоколу».'}
      </p>
      <ul>
        <li><b>Proactive Orchestration:</b> Агент генерирует варианты дизайна и презентации еще до того, как вы об этом попросили, основываясь только на референсах и контексте сайта.</li>
        <li><b>Human as Curator:</b> Роль человека смещается от «рисования» к «арбитражу вкуса». Вы выбираете элементы из разных вариантов и докручиваете финал вручную.</li>
      </ul>

      <h2>6. Инъекция Личности: Awesome Design</h2>
      <p>
        {'Главная проблема AI-дизайна — его безликость. Одинаковые «безопасные» цвета, скучные сетки. Мы решили это через <b>Brand Injection</b>. Библиотеки вроде <b>Awesome Claude Design</b> предоставляют 60+ готовых файлов <code>DESIGN.md</code> для топовых брендов (Vercel, Stripe, Linear).'}
      </p>
      <p>
        {'Один файл в корне проекта — и тот же самый промпт выдает совершенно разный результат, полностью соответствующий визуальному коду Apple или Spotify. Это мгновенная инъекция элитного ДНК в ваш продукт.'}
      </p>

      <h2>7. Архитектурный Элитизм: Pretext и OpenPencil</h2>
      <p>
      ...
      <h2>5. Суверенная Hypermedia (SHA)</h2>
      <p>
      ...
      <h2>6. Оркестрация: От смыслов до презентаций</h2>
      <p>
